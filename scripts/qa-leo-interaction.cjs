...
(async () => {
 ...
 try {
  ...
  assert.equal(await t.page.evaluate(()=>leoCalls[0].seen),'true');assert.equal(await t.page.evaluate(()=>leoEvents.filter(x=>x==='leo_peek_shown').length),1);
  ...
  t=await setup();await t.page.evaluate(()=>{const a=document.createElement('button');a.textContent='Other fixed control';Object.assign(a.style,{position:'fixed',right:'20px',bottom:'100px',width:'144px',height:'160px'});document.body.append(a);});await t.page.clock.runFor(10000);assert.equal(await t.count('PEEK'),0);await finish(t,'desktop action collision suppressed');
